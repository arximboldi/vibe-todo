...
void from_json(const nlohmann::json& j, AppState& state)
{
    ...
    if (j.contains("todos") && j["todos"].is_array()) {
        ...
        std::vector<TodoItem> todos_vec =
            j.at("todos").get<std::vector<TodoItem>>();
        // Convert std::vector to immer::flex_vector
        state.todos =
            immer::flex_vector<TodoItem>(todos_vec.begin(), todos_vec.end());
    } else {
        // Use spdlog eventually (warning)
        std::cerr << "Warning: State file format invalid or missing 'todos'. "
                     "Loading empty list."
                  << std::endl;
...
std::filesystem::path get_default_data_path()
{
    ...
    if (home) {
        data_dir = std::filesystem::path(home) / "Library" /
                   "Application Support" / app_name;
    } else {
        data_dir = std::filesystem::current_path() / app_name; // Fallback
    }
#else // Linux (and other Unix-like)
    // Linux: ~/.config/TuiTodoCpp/todos.json or
    // $XDG_CONFIG_HOME/TuiTodoCpp/todos.json
    ...
    try {
        ...
        std::cerr << "Error creating data directory " << data_dir.string()
                  << ": " << e.what() << std::endl;
        // Fallback to current directory if creation fails
        data_dir = std::filesystem::current_path(); // Use current dir, not
                                                    // inside app_name subdir
    }
...
std::optional<AppState> load_state(const std::filesystem::path& path)
{
    ...
    try {
        ...
        // Use spdlog eventually
        std::cerr << "JSON parsing error: " << e.what() << " at byte " << e.byte
                  << std::endl;
        return std::nullopt; // Return empty optional on parsing error
    } catch (const nlohmann::json::exception&
                 e) { // Catch other json errors (e.g., type errors)
        // Use spdlog eventually
